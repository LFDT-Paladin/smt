...
import (
	...
	"github.com/LFDT-Paladin/smt/internal/sparse-merkle-tree/storage"
...
func (s *MerkleTreeTestSuite) TestGenerateProof() {
	...
	hasher := &hash.PoseidonHasher{}
	db := storage.NewSqlStorage(provider, "test_1", hasher)
	mt, _ := NewMerkleTree(ctx, db, levels)
	...
	proofs, foundValues, err := mt.GenerateProofs(ctx, []*big.Int{target1, target2}, mt.Root())
	assert.NoError(s.T(), err)
...
func (s *MerkleTreeTestSuite) TestGenerateProofWithValue() {
	...
	proofs, foundValues, err := mt.GenerateProofs(context.Background(), []*big.Int{target1, target2}, mt.Root())
	assert.NoError(s.T(), err)
...
func (s *MerkleTreeTestSuite) TestVerifyProof() {
	...
	go func() {
		...
		root := mt.Root()
		p, _, err := mt.GenerateProofs(context.Background(), []*big.Int{target}, root)
		assert.NoError(s.T(), err)
